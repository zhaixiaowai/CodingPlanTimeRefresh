...
**Tech Stack:** Flutter (Dart) 桌面 stable；`window_manager`、`macos_window_utils`、`http`、`encrypt`、`path_provider`、`win32`；测试用内置 `flutter_test`。
...
用以下内容**完整替换** `codingplan_refresh/pubspec.yaml`：

```yaml
name: codingplan_refresh
description: 定时调用 LLM 并显示 API 用量百分比的桌面小工具（MAUI→Flutter 迁移版）
publish_to: 'none'
version: 1.0.0+1
...
### Task 2: AES-256-CBC 加解密工具（兼容性闸口）
...
class ConfigService {
  ...
  void _migrateFromOldPath() {
    ...
    final oldPath =
        '${Directory.current.path}${Platform.path.separator}data${Platform.path.separator}config.dat';
...
- [ ] **Step 3: 实现 localization_service.dart**

字符串集合从旧 `AppResources.resx`（中文）与 `AppResources.en.resx`（英文）平移。键采用 camelCase。创建 `codingplan_refresh/lib/services/localization_service.dart`：
...
class LocalizationService {
  ...
  static const _table = <String, Map<String, String>>{
    'manualTrigger': {'zh': '手动触发', 'en': 'Trigger'},
    'manualTriggerPopup': {'zh': '手动触发', 'en': 'Trigger'},
    'waitingPlaceholder': {'zh': '等待中…', 'en': 'Waiting…'},
    'resultHeader': {'zh': '结果', 'en': 'Result'},
    'save': {'zh': '保存', 'en': 'Save'},
    'cancel': {'zh': '取消', 'en': 'Cancel'},
    'token5hLabel': {'zh': '5小时Token', 'en': '5h Token'},
    'tokenWeekLabel': {'zh': '周Token', 'en': 'Weekly Token'},
    'mcpMonthLabel': {'zh': 'MCP月', 'en': 'MCP Month'},
    ...
    'errorMessage': {'zh': '错误：%s', 'en': 'Error: %s'},
    'apiUrlNotConfigured': {'zh': 'API URL 未配置', 'en': 'API URL not configured'},
    'apiKeyNotConfigured': {'zh': 'API Key 未配置', 'en': 'API Key not configured'},
    'apiCallFailed': {'zh': 'API 调用失败：%s', 'en': 'API call failed: %s'},
    'windowTitle': {'zh': '用量 %d%%%s', 'en': 'Usage %d%%%s'},
    'nextTriggerFormat': {'zh': '下次触发：%s（%d分%d秒后）', 'en': 'Next: %s (%dm %ds)'},
    'resetToday': {'zh': '今日 %s 重置', 'en': 'Resets today %s'},
    'resetOther': {'zh': '%s 重置', 'en': 'Resets %s'},
    // jokePrompt：发送给 LLM 的提示词（中文版/英文版）
    'jokePrompt': {
      'zh': '讲一个程序员笑话，简短有趣，不超过三句话',
      'en': 'Tell a short programmer joke, max three sentences'
    },
  };
...
> **平移校验点**：实现前需打开旧 `CodingPlanTimeRefresh/Resources/Strings/AppResources.resx` 与 `AppResources.en.resx`，逐条核对上述键值是否与旧文案完全一致；若有出入，以旧 resx 为准修正 `_table`。
...
### Task 10: 窗口控制（window_manager + macos_window_utils）
...
**Interfaces:**
- Consumes: `package:window_manager`、`package:macos_window_utils`（macOS）
...
class WindowController {
  ...
  /// 禁 macOS zoom 按钮由原生 MainFlutterWindow.swift 实现（见 Step 2），Dart 侧不调用。
  Future<void> setup({
...
- [ ] **Step 2: macOS 原生回退（禁 zoom）**

若 `macos_window_utils` 未直接提供「禁 zoom」API，则在 `codingplan_refresh/macos/Runner/MainFlutterWindow.swift` 的 `contentViewdidArrangeSubviews` 或 `awakeFromNib` 中注入原生调用。编辑该文件，在 `MainFlutterWindow` 类内加入：
...
class MainFlutterWindow: NSWindow {
    override func awakeFromNib() {
        ...
        self.styleMask.remove(.fullScreenBarButton) // 视效果可选
        super.awakeFromNib()
    }
}
```

> 实现者需在 Mac 上 build 验证：窗口不出现可用的绿色最大化（zoom 无效）。
...
期望：无平台相关编译错误（macOS 分支在 Windows 上被 `Platform.isMacOS` 守卫）。
...
期望：窗口出现，绿色 zoom 按钮点击无效/不可用。

- [ ] **Step 5: Commit**

```bash
git add codingplan_refresh/lib/platform/window_controller.dart codingplan_refresh/macos/Runner/MainFlutterWindow.swift
git commit -m "feat(platform): 窗口控制封装（固定尺寸/居中/置顶/禁 zoom）"
```
...
/// 单实例：尝试独占 bind 本地端口；失败=已有实例在跑。
/// 纯 dart:io 跨平台可编译，进程退出时端口自动释放，无需 win32 条件导入。
/// （对 spec §6 的「Windows mutex / macOS 文件锁」收敛为端口探测等价方案。）
class SingleInstance {
  // 选一个不常用端口；若与其它程序冲突可调整。
  static const int _port = 51397;
  ServerSocket? _socket;

  /// 返回 true=当前是首实例可继续；false=已有实例，应退出。
  Future<bool> ensure() async {
    try {
      _socket = await ServerSocket.bind('127.0.0.1', _port);
      return true;
    } catch (_) {
      return false;
    }
...
端口方案不依赖 `win32`（pubspec 已不含该依赖）。验证：
...
期望：第二次进程因 bind 端口 51397 失败而立即退出，只保留一个窗口。
  ...
  Future<void> _callLlm({required bool manual, int retry = 1}) async {
    ...
    try {
      ...
      await widget.llm.askStream(
        ...
        question: prompt,
        onChunk: (c) => setState(() {
          if (_resultText == widget.l10n.t('loading')) _resultText = '';
          _resultText += c;
        }),
      );
      _resultHeader = widget.l10n.t('resultHeader');
...
Future<void> main() async {
  ...
  // 单实例
  final first = await SingleInstance().ensure();
  if (!first) {
    exit(0);
  }
...
```
T1 脚手架
  └─ T2 AES（兼容性闸口①：能解密旧 config.dat 才继续）
